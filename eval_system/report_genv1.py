...
class WorksheetValidator:
    ...
    def _extract_letter(
        ...
        # Majority threshold based on actual voter count
        num_voters = len(voter_rows)
        majority   = (num_voters // 2) + 1  # e.g. 3 voters → 2, 4 voters → 3

        # Find which repetitions majority voted as incorrect
        incorrect_indices = []
    ...
    def calculate_confusion_matrix(
        ...
        # Build AI predictions: { 'A': [1, 3], 'b': [2] }
        ...
        # All letters present in either ground truth or AI predictions
        all_letters = set(ground_truth.keys()) & set(ai_predictions.keys())
...
if __name__ == '__main__':
    validator = WorksheetValidator('VotingPage.xlsx')
    ...
    letter_instances = [
        {'letter': 'a', 'repetition_num': 1, 'letter_form': 100.0, 'size': 100.0, 'line_align': 53.0},
        {'letter': 'a', 'repetition_num': 2, 'letter_form': 100.0, 'size': 100.0, 'line_align': 58.0},
        {'letter': 'a', 'repetition_num': 3, 'letter_form': 100.0, 'size': 100.0, 'line_align': 58.0},
        {'letter': 'a', 'repetition_num': 4, 'letter_form': 100.0, 'size': 100.0, 'line_align': 53.0},
        {'letter': 'b', 'repetition_num': 1, 'letter_form': 100.0, 'size': 100.0, 'line_align': 85.0},
        {'letter': 'c', 'repetition_num': 1, 'letter_form': 100.0, 'size': 100.0, 'line_align': 58.0},
        {'letter': 'k', 'repetition_num': 2, 'letter_form': 100.0, 'size': 100.0, 'line_align': 68.0},
    ]

    # Print voting + confusion matrix together
    validator.print_validation_report({5: letter_instances})

    # Generate DataFrame report
    report_df = validator.generate_validation_report({5: letter_instances})
